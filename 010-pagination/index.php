...
// config
$limit = 5;
$jumlah = count(read("SELECT id FROM siswa"));
$page = ceil($jumlah / $limit);
$page_now = (isset($_GET['page'])) ? $_GET['page'] : 1;
$first = ($limit * $page_now) - $limit;

$students = read("SELECT * FROM siswa ORDER BY id DESC LIMIT $first, $limit");
...
<body>
    ...
    <div>
        ...
        <?php if($page_now > 1) : ?>
            <button><a href="?page=<?= $page_now - 1 ?>">&laquo;</a></button>
        <?php
        endif;
        for($i = 1; $i <= $page; $i++) : ?>
        ...
        if($page_now < $page) : ?>
            <button><a href="?page=<?= $page_now + 1 ?>">&raquo;</a></button>
        <?php endif; ?>
    </div><br>
